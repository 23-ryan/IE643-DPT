...
from datasets.brain import NEW_CNAMES as BRAIN_CNAMES
from datasets.caltech101 import NEW_CNAMES as CALTECH_CNAMES
...
CUSTOM_TEMPLATES = {
    ...
    "Brain": "a photo of a {} scan in brain.",
}
...
class CAVPT(nn.Module):
    ...
    def forward(self, class_token, class_prompt, i):
        ...
        # class_prompt ==> top K selected prompts
        # class_token ==> all the input image features
...
class Transformer_VPTD(nn.Module):
    ...
    def forward(self, x, text_feature):
        ...
        # ================================== #
        # ================================== #
        #### Combining learned visual prompts with visual image features
        for i in range(self.bottom_limit):
            x = torch.cat([x, ctx[i]], dim=0)
            x, _ = self.resblocks[i](x)
            x = x[:-n_ctx, :, :]
        # ================================== #
        # ================================== #
        ...
        for i in range(self.layers-self.bottom_limit):
            ...
            # Extract the cross attention features from CAVPT
            ...
            # ================================== #
            # ================================== #
            # x ==> the inputs image features + CAVPT cross attention features
            # | | | | | | | 
            # The big transformer attention block 
            x = torch.cat([x, class_prompt], dim=0)
            x, attentions = self.resblocks[i+self.bottom_limit](x)
            # ================================== #
            # ================================== #
...
class ImageEncoder_VPTD(nn.Module):
    ...
    def forward(self, x, text_feature):
        ...
        # class_embedding is image class token.

        ### getting the combined image featues (image patch embedding + image class token embedding) 
        ...
        # ================================== #
        # Transformer + CAVPT #
        # "(image-patch-embeddings, image-class-token and selected-text-features[via-Selector])"
        x, layer_logits, weights = self.transformer(x, text_feature)
        x = x.permute(1, 0, 2)  # LND -> NLD
        # ================================== #
        # ================================== #
...
class CustomCLIP_Selected_CoVPTDeep(nn.Module):
    def __init__(self, cfg, classnames, clip_model, devices):
        ...
        # Generating the combined tokenized text prompts
        prompts = torch.cat([clip.tokenize(p) for p in prompts])
        clip_model.to(devices)
        prompts = prompts.to(devices)

        # Generating the initial text features which will be used for other purposes
        with torch.no_grad():
        ...
        # Prompt Learner Module which is aimed at learning text prompts
        self.prompt_learner = PromptLearner(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
    ...
    def forward(self, image):
        ...
        # ================================== #
        # ================================== #
        #### Here we are finding the top-K text features, which will be further used for training of the CAVPT module
        # to generate class aware visual prompts
        with torch.no_grad():
            zeroshotclip_image_feature, zershotclip_weights = self.zeroshot_clip_image_encoder(image.type(self.dtype))
        ...
        # Encoding Image
        image_features, layer_logits, weights = self.image_encoder(image.type(self.dtype), text_features[indices])
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        # ================================== #
        # ================================== #
        ...
        # ================================== #
        # ================================== #
        # Finding Cross-Similarities
        # prompted text features + prompted image features
        logit_scale = self.logit_scale.exp()
        logits = logit_scale * image_features @ text_features_norm.t()
        
        # zeroshotclip_image features + prompted text features
        logits1 = logit_scale * zeroshotclip_image_feature @ text_features_norm.t()
        
        # Prompted image features + zeroshotclip text features
        logits2 = logit_scale * image_features @ self.text_features.t()
        # ================================== #
        # ================================== #

        return logits, layer_logits, indices, logits1, logits2, weights
# end
...
class DPT(TrainerX):
    ...
    def model_inference(self, input, label=None):
        ...
        # generate the heaptmap for the given input image and model
        ...
        # Plot the attention map along with image highlighting the important features of the image
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            real_label = label
            output, layer_logits, indices, output1, output2, weights = self.model(image)
            ...
            # At last we operate the backward step for all of the loss functions computed above
